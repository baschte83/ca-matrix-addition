...
matadd:
	PUSH	{r4,r5,r6,r7,r8,r9,lr}		@ save callee-save registers to stack

	LSL	r6, r3, #2			@ shift height by 2 so it is multiplied by 4
	SUB	r6, r6, #4
	LDR	r4, [sp, #28]			@ load width of matrizes in r4
	LSL	r4, r4, #2			@ shift width by 2 so it is multiplied by 4 
	STR	r4, [sp, #28]			@ stores height to stack

loop_rows:					@ loop to iterate through all rows of a matrix
	LDR	r7, [sp, #28]			@ initialize columns-offset with width
...
loop_columns:					@ loop to iterate through all columns of a matrix
	...
	POP	{r4,r5,r6,r7,r8,r9,lr}	@ restore callee-save registers from stack
